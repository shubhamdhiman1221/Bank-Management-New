...
def pwd_rst_wrt(master,pin_new,accnt):
	cursor=dbstore.cursor()
	cursor.execute("UPDATE Accnt_Record_new SET pin = (?) where account_no = (?)",(pin_new,accnt,))
	#	INSERT INTO transaction_new (account_no,credit,debit,Date) values (?,?,?,?)",(accnt2,amt,0,time_now))
	dbstore.commit()

	messagebox.showinfo("Operation Successfull!!","Password reset Successful!!")
	master.destroy()
	return
...
def pwd_rst (accnt):
	...
	#l2.pack(side="top")
	#e2.pack(side="top")
	l1=tk.Label(creditwn,relief="raised",font=("Times",16),text="Enter the new password: ")
	e1=tk.Entry(creditwn,relief="raised")
	l1.pack(side="top")
	e1.pack(side="top")

	b=tk.Button(creditwn,text="Change",font=("Times",16),relief="raised",command=lambda:pwd_rst_wrt(creditwn,e1.get(),accnt))
	b.pack(side="top")
	creditwn.bind("<Return>",lambda x:pwd_rst_wrt(creditwn,e1.get(),accnt))
...
def logged_in_menu(accnt,name):
	...
	img8=tk.PhotoImage(file="pwd_reset.gif")
	...
	b7.image=myimg7
	b8=tk.Button(image=myimg8,command=lambda: pwd_rst(accnt))
	b8.image=myimg8
